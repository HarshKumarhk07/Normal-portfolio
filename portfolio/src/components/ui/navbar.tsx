...
export const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-1/2 -translate-x-1/2 z-[999] w-fit max-w-[calc(100vw-2rem)] md:max-w-none transition-all duration-1000 ease-[cubic-bezier(0.16,1,0.3,1)] ${
        visible ? "translate-y-5 opacity-100" : "-translate-y-full opacity-0"
      }`}
    >
      <div className="relative flex items-center justify-center gap-1 overflow-x-auto no-scrollbar md:overflow-visible px-6 md:px-8 py-3 rounded-full bg-black/85 border border-white/10 shadow-lg group/nav">
        {/* Creative Side Accents */}
        ...
        {navItems.map((item) => (
          <a
            key={item.label}
            href={item.href}
            className="flex items-center shrink-0 px-3 md:px-4 py-2 text-[10px] font-bold tracking-[0.15em] text-white/70 hover:text-white transition-all duration-300 relative group active:scale-95"
          >
