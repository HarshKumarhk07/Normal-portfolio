...
import { Mail, Linkedin, Github, Code2, Phone, ArrowRight, Download } from "lucide-react";
...
export default function Home() {
  return (
    <main id="home" className="relative w-full bg-[#050505] text-[#ededed] overflow-x-hidden">
      
      <Navbar />

      {/* Redesigned Hero Section */}
      <section className="relative min-h-screen w-full flex items-center justify-center overflow-hidden pt-32 pb-20">
        <div className="max-w-[1200px] mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center gap-12 md:gap-24 relative z-20">
          
          {/* Left Side: Profile Code Box */}
          <div className="w-full md:w-[45%] flex justify-center md:justify-start">
            <div className="relative group">
              {/* Decorative Icons around the box */}
              <div className="absolute -top-6 -right-6 p-3 rounded-xl bg-white/5 border border-white/10 backdrop-blur-sm shadow-xl z-30 group-hover:bg-blue-500/10 group-hover:border-blue-500/40 transition-all duration-500 hidden md:block">
                <Code2 size={24} className="text-blue-400" />
              </div>
              <div className="absolute -bottom-6 -left-6 p-3 rounded-xl bg-white/5 border border-white/10 backdrop-blur-sm shadow-xl z-30 group-hover:bg-blue-500/10 group-hover:border-blue-500/40 transition-all duration-500 hidden md:block">
              ...
              <div className="relative">
                ...
                <div className="absolute inset-0 bg-blue-500/20 blur-[100px] rounded-full -z-10 opacity-50 group-hover:opacity-80 transition-opacity duration-1000" />
                <ProfileCodeBox />
              </div>
            </div>
          </div>
          ...
          <div className="w-full md:w-[55%] flex flex-col items-center md:items-start text-center md:text-left">
            <Reveal delay={0.1}>
              <div className="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 px-4 py-1.5 rounded-full mb-8">
                ...
                <span className="text-blue-400 text-[10px] tracking-[0.2em] font-bold uppercase">System Online</span>
              </div>
            </Reveal>

            <Reveal delay={0.2}>
              <h1 className="text-white font-black text-6xl md:text-8xl tracking-tighter leading-[0.9] mb-6">
                Devkaran Singh Tanwar
              </h1>
            </Reveal>

            <Reveal delay={0.3}>
              <div className="flex items-center gap-3 text-3xl md:text-4xl font-light text-white/50 mb-8">
                <span>I&apos;m a</span>
                <span className="text-blue-400 font-semibold relative flex items-center">
                  Full Stack
                  <motion.div 
                    animate={{ opacity: [1, 0] }}
                    transition={{ duration: 0.8, repeat: Infinity, ease: "linear" }}
                    className="w-[3px] h-[1.1em] bg-blue-500 ml-2"
                  />
                </span>
              </div>
            </Reveal>
            ...
            <Reveal delay={0.5} className="flex flex-col sm:flex-row gap-6 w-full sm:w-auto">
              <a href="#projects" className="bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm tracking-widest uppercase px-10 py-5 rounded-xl transition-all shadow-lg shadow-blue-500/20 flex items-center justify-center gap-3 active:scale-95">
                View Projects <ArrowRight size={18} />
              </a>
              <a href="#contact" className="border border-white/10 bg-white/5 hover:bg-white/10 text-white font-bold text-sm tracking-widest uppercase px-10 py-5 rounded-xl transition-all flex items-center justify-center gap-3 active:scale-95">
                Contact Me <Mail size={18} />
              </a>
            </Reveal>

            <Reveal delay={0.6} className="mt-16 flex items-center gap-8 border-t border-white/5 pt-8 w-full md:w-auto justify-center md:justify-start">
               {[
                 { icon: Github, href: "https://github.com/HarshKumarhk07" },
                 { icon: Linkedin, href: "https://linkedin.com/in/harsh-kumar-01/" },
                 { icon: Mail, href: "mailto:harshkumarhk525@gmail.com" }
               ].map((social, i) => (
                 <a key={i} href={social.href} target="_blank" rel="noopener noreferrer" className="text-white/30 hover:text-white transition-all duration-300">
                   <social.icon size={24} />
                 </a>
               ))}
            </Reveal>
        ...
        <div className="absolute top-0 left-0 w-full h-full -z-10 pointer-events-none">
          ...
          <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-indigo-500/5 blur-[100px] rounded-full" />
        </div>
      </section>
        ...
        <div className="relative z-10 w-full bg-transparent">
              ...
              <div className="w-full md:w-[60%] flex flex-col">
                ...
                <Reveal delay={0.2}>
                  <p className="text-white/55 text-lg leading-[1.85] max-w-xl font-light">
                    ...
                    with a CGPA of <span className="text-white">7.76</span>. I build full-stack MERN applications that are 
          ...
          {/* SECTION 6: CONTACT CTA */}
           <section id="contact" className="min-h-[70vh] w-full flex flex-col items-center justify-center relative border-t border-white/[0.08]">
            ...
            <div 
              className="absolute bottom-10 w-full text-center text-white/15 hover:text-white/30 transition-all duration-1000 text-[9px] md:text-[10px] uppercase font-medium z-10 px-6 cursor-default tracking-[0.4em]"
            >
              DEVKARAN SINGH TANWAR <span className="mx-2 md:mx-4">·</span> MERN DEVELOPER <span className="mx-2 md:mx-4">·</span> LPU <span className="mx-2 md:mx-4">·</span> 2025
            </div>
          </section>
          </div>
        </div>
